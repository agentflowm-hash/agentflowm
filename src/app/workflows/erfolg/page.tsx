...
function ErfolgContent() {
  ...
  const botName = searchParams.get("bot") || "Ihr Bot";
  const purchaseType = searchParams.get("type") || "rent";
  const price = searchParams.get("price") || "0";

  const isRent = purchaseType === "rent";
  const priceLabel = isRent ? `${price} € / Monat` : `${price} € einmalig`;
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span className="text-white/60">Preis:</span>
          ...
          >
            ...
            <ul className="space-y-3">
              {[
                {
                  icon: "📧",
                  text: "Sie erhalten in Kürze eine Bestätigungs-E-Mail",
                },
                {
                  icon: "📞",
                  text: "Wir kontaktieren Sie innerhalb von 24h zur Einrichtung",
                },
                {
                  icon: "🔗",
                  text: "Gemeinsam verbinden wir Ihre Tools mit dem Bot",
                },
                {
                  icon: "✅",
                  text: "Nach der Einrichtung läuft Ihr Bot automatisch 24/7",
                },
              ].map((step, i) => (
                <motion.li
                  key={i}
          ...
          >
            <span className="text-white/70">Fragen?</span> Kontaktieren Sie uns
            jederzeit unter{" "}
            <a
