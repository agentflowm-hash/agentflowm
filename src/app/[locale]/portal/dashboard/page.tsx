    ...
    return (
      <div className="min-h-screen bg-[#030308] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/50 text-lg">{t("loading")}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#030308] flex items-center justify-center p-4">
        <div className="portal-card text-center max-w-md">
          ...
          <h2 className="text-xl font-semibold text-white mb-2">{t("loadError")}</h2>
          <p className="text-white/50 mb-6">{error || t("somethingWrong")}</p>
          <button onClick={fetchData} className="portal-btn">{t("retry")}</button>
        </div>
      </div>
    );
  }
      ...
      <nav className="border-b border-white/[0.06] bg-[#030308]/80 backdrop-blur-xl sticky top-[65px] z-10">
        <div className="max-w-5xl mx-auto px-4">
          <div className="flex gap-1 -mb-px">
            {[
              { id: "overview" as Tab, label: t("tabs.overview"), icon: "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" },
              { id: "messages" as Tab, label: t("tabs.messages"), icon: "M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z", badge: unreadCount },
              { id: "files" as Tab, label: t("tabs.files"), icon: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z", badge: files.length },
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        {activeTab === "overview" && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="portal-card">
              <h2 className="text-xl font-bold text-white mb-1">{t("welcome")}, {client.name.split(" ")[0]}!</h2>
              <p className="text-white/50">{t("projectStatus")}</p>
            </div>
            ...
            <div className="portal-card">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                  <p className="text-xs text-white/40 uppercase tracking-wider mb-1">{t("project")}</p>
                  <h2 className="text-xl font-bold text-white">{project.name}</h2>
                  <p className="text-white/50 mt-1">{t("package")}: <span className="text-[#FC682C] font-medium">{project.package}</span></p>
                </div>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-white/50">{t("progress")}</span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: t("projectStart"), value: project.startDate },
                  { label: t("goLive"), value: project.estimatedEnd },
                  { label: t("milestones"), value: `${completedMilestones}/${milestones.length}` },
                  { label: t("contactPerson"), value: project.manager },
                ].map((item) => (
            ...
            {/* Timeline */}
            <div className="portal-card">
              <h3 className="text-lg font-bold text-white mb-4">{t("timeline")}</h3>
        ...
        {activeTab === "messages" && (
          <div className="portal-card animate-fade-in">
            <h3 className="text-lg font-bold text-white mb-4">{t("messages.title")}</h3>

            <div className="space-y-3 mb-4 max-h-[400px] overflow-y-auto">
              {messages.length === 0 ? (
                <p className="text-center text-white/40 py-8">{t("messages.noMessages")}</p>
              ) : (
                messages.map((msg) => (
        ...
        {activeTab === "files" && (
          <div className="portal-card animate-fade-in">
            <h3 className="text-lg font-bold text-white mb-4">{t("files.title")}</h3>

            {files.length === 0 ? (
              <p className="text-center text-white/40 py-8">{t("files.noFiles")}</p>
            ) : (
        ...
        <div className="max-w-5xl mx-auto px-4 py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
            <Link href="/" className="text-sm text-white/40 hover:text-white transition-colors">
              ← {t("footer.backToWebsite")}
            </Link>
            <p className="text-sm text-white/40">
              {t("footer.questions")} <a href="mailto:kontakt@agentflowm.com" className="text-[#FC682C] hover:underline">kontakt@agentflowm.com</a>
            </p>
          </div>
        </div>
      </footer>
