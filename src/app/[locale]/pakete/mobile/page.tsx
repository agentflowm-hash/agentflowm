...
export default function MobileAppPackagePage() {
  ...
  const t = useTranslations("pages.pakete.mobile");
        ...
        <div className="container relative z-10 px-4 sm:px-6">
          <div className="max-w-4xl mx-auto">
            ...
            >
              ...
              <span className="text-xs font-medium text-[#8B5CF6]">{t("hero.badge")}</span>
            </div>
            ...
            >
              ...
              <span className="text-white/70 text-2xl sm:text-3xl lg:text-4xl ml-2">{t("hero.titleSuffix")}</span>
            </h1>
            ...
            >
              {[0, 1].map((i) => (
                <div key={i} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/10">
                  <span className="text-xl">{platformIcons[t(`platformsList.${i}.icon`)] || "📱"}</span>
                  <div>
                    <div className="text-sm font-semibold text-white">{t(`platformsList.${i}.title`)}</div>
                    <div className="text-[10px] text-white/50">{t(`platformsList.${i}.desc`)}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            >
              <div>
                <div className="text-4xl font-bold text-[#8B5CF6]">{t("hero.price")} €</div>
                <div className="text-sm text-white/50">{t("hero.priceLabel")}</div>
              </div>
              <div className="h-12 w-px bg-white/10" />
              <div>
                <div className="text-sm text-white/70">{t("hero.included")}</div>
                <div className="text-lg font-semibold text-white">{t("hero.platforms")}</div>
              </div>
            </div>
        ...
        <div className="container px-4 sm:px-6">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-white/60 mb-8">
              {t("storeReady.description")}
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <div className="p-6 rounded-xl bg-white/[0.03] border border-white/[0.08] flex-1 max-w-xs">
                <span className="text-4xl mb-3 block">🍎</span>
                <h3 className="font-semibold text-white mb-2">{t("storeReady.appStore.title")}</h3>
                <p className="text-sm text-white/60">{t("storeReady.appStore.desc")}</p>
              </div>
              <div className="p-6 rounded-xl bg-white/[0.03] border border-white/[0.08] flex-1 max-w-xs">
                <span className="text-4xl mb-3 block">▶️</span>
                <h3 className="font-semibold text-white mb-2">{t("storeReady.googlePlay.title")}</h3>
                <p className="text-sm text-white/60">{t("storeReady.googlePlay.desc")}</p>
              </div>
            </div>
          </div>
        </div>
