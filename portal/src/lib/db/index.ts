...
function initializeTables(db: Database.Database) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_approvals_project ON portal_approvals(project_id);
    CREATE INDEX IF NOT EXISTS idx_approvals_status ON portal_approvals(status);
  `);
  ...
  // Migration: Add preview columns to portal_projects
  try {
    db.exec(`ALTER TABLE portal_projects ADD COLUMN preview_url TEXT`);
  } catch {
    // Column already exists
  }
  try {
    db.exec(
      `ALTER TABLE portal_projects ADD COLUMN preview_enabled INTEGER NOT NULL DEFAULT 0`,
    );
  } catch {
    // Column already exists
  }
...
export function getProjectApprovals(projectId: number): PortalApproval[] {
  const db = getSqlite();
  return db
    .prepare(
      `
  ...
  `,
    ...
    .all(projectId) as PortalApproval[];
}
